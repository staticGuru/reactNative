...
const Mentions = _a => {
  var {
      ...
      mentionTextStyle = utils_1.defaultMentionTextStyle,
      containerStyle,
      onSelectionChange,
    } = _a,
    textInputProps = __rest(_a, [
      'value',
      'onChange',
      'renderSuggestions',
      'trigger',
  ...
  const {plainText, parts} = react_1.useMemo(
    () => utils_1.getPartsFromValue(trigger, value),
    [value],
  );
  ...
  const onChangeInput = changedText => {
    onChange(
      utils_1.generateValueFromPartsAndChangedText(
        parts,
        plainText,
        changedText,
      ),
  ...
  const onMentionSuggestionPress = suggestion => {
    ...
    const spacePartIndex = currentPart.text.lastIndexOf(
      ' ',
      selection.end - currentPart.position.start - 1,
    );
      ...
      react_1.default.createElement(
        ...
        parts.map(({text, data}, index) =>
          data
            ? react_1.default.createElement(
                react_native_1.Text,
                {key: `${index}-m`, style: mentionTextStyle},
                text,
              )
            : react_1.default.createElement(
                react_native_1.Text,
